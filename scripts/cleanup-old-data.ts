...
const { VITE_SUPABASE_URL: supabaseUrl, VITE_SUPABASE_PUBLISHABLE_KEY: supabaseServiceKey } = process.env
...
async function main() {
  ...
  try {
    ...
    if (votesError) {
      ...
      console.log(`Successfully deleted ${deletedVotes?.length || 0} old votes.`)
    }
    ...
    if (playersError) {
      ...
      console.log(`Successfully deleted ${deletedPlayers?.length || 0} old players.`)
    }
    ...
    if (roomsError) {
      ...
      console.log(`Successfully deleted ${deletedRooms?.length || 0} old rooms.`)
    }
    ...
    if (usersError) {
      ...
      const oldUsers = users.users.filter(user =>
        DateTime.fromISO(user.created_at) < DateTime.now().minus({ hours: 24 })
      )
